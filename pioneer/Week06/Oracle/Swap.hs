...
--------------------------------------------
-- Oracle: The Oracle to use
-- Address: The address/hash of the Oracle
-- PubKeyHash: (Dataum) Pubkey hash of the seller (person selling Ada in exchange for USD )

--------------------------------------------
mkSwapValidator :: Oracle -> Address -> PubKeyHash -> () -> ScriptContext -> Bool
mkSwapValidator oracle addr pkh () ctx =
    txSignedBy info pkh ||                                                      -- If either signed by swap owner (if they wish to withdraw their funds) OR
    (traceIfFalse "expected exactly two script inputs" hasTwoScriptInputs &&    -- Transaction passed validation tests
     traceIfFalse "price not paid"                     sellerPaid
     )

  where
    info :: TxInfo                                                              -- Get transaction info function
    info = scriptContextTxInfo ctx                                              -- Get tranasaction info from the context

    oracleInput :: TxOut                                                        -- Function for getting the Ouput from the Oracle provided to be used as input in this script
    oracleInput =                                                               
      let                                                                       -- A let expression allows you to combine the readability of a where clause with the power of your lambda function
        ins = [ o                                                               -- Define let variables
              | i <- txInfoInputs info
              , let o = txInInfoResolved i
              , txOutAddress o == addr
              ]
      in
        case ins of                                                              -- main expression: 
            [o] -> o                                                             -- check there is only one valid oracle
...
typedSwapValidator oracle = Scripts.mkTypedValidator @Swapping
    ($$(PlutusTx.compile [|| mkSwapValidator ||])
        `PlutusTx.applyCode` PlutusTx.liftCode oracle                          -- Provide the Oracle to the validator function
        `PlutusTx.applyCode` PlutusTx.liftCode (oracleAddress oracle))         -- Provide the Oracle address to the validator function
